...
import { useAdminUser, useToggleStatus, useToggleBan, useDeleteUser, useChangeRole } from '@/hooks/useAdmin';
...
export default function AdminUserDetail() {
  ...
  const statusBadges: Array<{ label: string; active: boolean; style: string }> = [
    { label: user.isActive ? 'Active' : 'Disabled', active: user.isActive, style: user.isActive ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400' },
    { label: 'Banned', active: !!user.bannedAt, style: 'bg-red-900/30 text-red-400' },
    { label: 'Deleted', active: !!user.deletedAt, style: 'bg-stone-800 text-stone-400' },
  ];

  return (
    <div>
      <Link href="/admin/users" className="mb-4 inline-block text-sm text-amber-500 hover:text-amber-400">
        &larr; Back to users
      </Link>
      ...
      <div className="mt-2 rounded-lg border border-stone-800 bg-stone-900 p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-500">@{user.username} &middot; {user.email}</p>
            <div className="mt-3 flex gap-2">
              {statusBadges.filter((b) => b.active || b.label === 'Active' || b.label === 'Disabled').map((b) => (
                <span key={b.label} className={`inline-block rounded px-2 py-0.5 text-xs ${b.style}`}>
                  {b.label}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="ghost" onClick={() => toggleStatus.mutate({ id: userId, isActive: !user.isActive })}>
              {user.isActive ? 'Disable' : 'Enable'}
            </Button>
            <Button variant="ghost" onClick={() => toggleBan.mutate({ id: userId, banned: !user.bannedAt })}>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-stone-800 bg-stone-900 p-5">
          <h2 className="mb-3 text-sm font-bold text-stone-100">Profile</h2>
          {user.profile ? (
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-stone-500">Avatar</dt>
                <dd className="text-stone-300">{user.profile.avatarUrl ? 'Yes' : 'No'}</dd>
              </div>
              <div className="flex justify-between">
            ...
            <p className="text-sm text-stone-500">No profile data.</p>
          )}
        </div>

        <div className="rounded-lg border border-stone-800 bg-stone-900 p-5">
          <h2 className="mb-3 text-sm font-bold text-stone-100">Statistics</h2>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-stone-500">Guest Conversions</dt>
              <dd className="text-stone-300">{user._count.convertedGuests}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-stone-500">Email Verified</dt>
              <dd className="text-stone-300">{user.emailVerifiedAt ? new Date(user.emailVerifiedAt).toLocaleDateString() : 'No'}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-stone-500">Last Login</dt>
              <dd className="text-stone-300">{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString() : 'Never'}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-lg border border-stone-800 bg-stone-900 p-5">
        <h2 className="mb-3 text-sm font-bold text-stone-100">Timeline</h2>
        <dl className="space-y-2 text-sm">
          <div className="flex justify-between">
          ...
          {user.bannedAt && (
            <div className="flex justify-between">
              <dt className="text-stone-500">Banned</dt>
              <dd className="text-red-400">{new Date(user.bannedAt).toLocaleString()}</dd>
            </div>
          )}
          {user.deletedAt && (
            <div className="flex justify-between">
              <dt className="text-stone-500">Deleted</dt>
