...
export default function AdminLiveGames() {
  ...
  if (isError) {
    return <p className="py-8 text-center text-sm text-red-400">Failed to load games.</p>;
  }

  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-stone-100">Live Games</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-stone-800 bg-stone-900">
              ...
              <th className="px-4 py-3 font-semibold text-stone-400">Player 1</th>
              <th className="px-4 py-3 font-semibold text-stone-400">Player 2</th>
          ...
          <tbody>
            {data?.games.map((game) => (
              <tr key={game.tableId} className="border-b border-stone-800 last:border-0 hover:bg-stone-900/50">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block rounded px-2 py-0.5 text-xs ${
                    ...
                    game.status === 'paused' ? 'bg-yellow-900/30 text-yellow-400' :
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex flex-wrap gap-1">
                      {game.status === 'paused' ? (
                        <Button
                          variant="ghost"
                          onClick={() => gameAction.mutate({ gameId: game.tableId, action: 'resume' })}
                        ...
                        <Button
                          variant="ghost"
                          onClick={() => gameAction.mutate({ gameId: game.tableId, action: 'pause' })}
                          disabled={gameAction.isPending}
                        >
                          Pause
                        </Button>
                      ...
                      <Button
                        variant="ghost"
                        onClick={() => gameAction.mutate({ gameId: game.tableId, action: 'terminate' })}
                        disabled={gameAction.isPending}
                      >
                        Terminate
                      </Button>
                      <Button
                        variant="ghost"
                        onClick={() => gameAction.mutate({ gameId: game.tableId, action: 'force-draw' })}
                        disabled={gameAction.isPending}
                      >
                        Force Draw
                      </Button>
                    ...
                    <div className="flex gap-1">
                      <span className="text-xs text-stone-500 mr-1 self-center">Resign:</span>
                      <select
                        value={resignPlayer[game.tableId] ?? 1}
                        onChange={(e) => setResignPlayer((prev) => ({ ...prev, [game.tableId]: Number(e.target.value) as 1 | 2 }))}
                      ...
                      <Button
                        variant="ghost"
                        onClick={() => gameAction.mutate({ gameId: game.tableId, action: 'force-resign', body: { player: resignPlayer[game.tableId] ?? 1 } })}
                        disabled={gameAction.isPending}
                      >
                        Go
                      </Button>
      ...
      {data?.games.length === 0 && (
        <p className="py-8 text-center text-sm text-stone-500">No active games.</p>
      )}
    </div>
  );
}
