...
export default function AdminBanned() {
  ...
  if (isError) {
    return <p className="py-8 text-center text-sm text-red-400">{t.admin.failedBanned}</p>;
  }
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-stone-100">{t.admin.bannedUsers}</h1>

      {users.length === 0 ? (
        <div className="rounded-lg border border-dashed border-stone-700 py-12 text-center">
          <p className="text-sm text-stone-500">{t.admin.noBanned}</p>
        </div>
      ) : (
